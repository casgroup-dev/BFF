    ...
    <template v-if="showEconomicalSection">
      ...
      <div class="row"
        ...
        <button :disabled="!canSignAndPublish"
                ...
                : 'Los resultados ya se encuentran publicados, no los puedes modificar.'}`"
...
export default {
  ...
  methods: {
    ...
    reduceDocumentsToType (users, type) {
      if (type !== 'technicals' && type !== 'economicals') {
        throw new Error(`Type must be 'economicals' or 'technicals'`)
      }
      return users.reduce((accumulatedResult, currentParticipant) => {
        if (
          currentParticipant.user.company &&
          currentParticipant.user.company.businessName !==
            'CAS compañía de asesorías y servicios SPA'
        ) {
          // If the current users' company is not in the accumulated result, add it
          const currentBusinessName =
            currentParticipant.user.company.businessName
          let providerDocuments = accumulatedResult.find(
            a => a.provider === currentBusinessName
          )
          if (!providerDocuments) {
            providerDocuments = {
              provider: currentBusinessName,
              documents: []
            }
            accumulatedResult.push(providerDocuments)
          }
          // Concat to the accumulated documents of the provider company
          providerDocuments.documents = providerDocuments.documents.concat(
            currentParticipant.documents[type]
          )
          // Set approved if any user is approved
          if (currentParticipant.approved.technically) {
            providerDocuments.approved = true
          }
        }
    ...
    approveProviders (providers) {
      ...
      api
        .approve(this.bidding.id, api.types.technically, providers)
        .then(() => {
          let message = providers.length
            ? `Proveedores aprobados técnicamente:<br>${providers
                .map(p => `- ${p}`)
                .join('<br>')}.`
    ...
    adjudicate (itemName, adjudications) {
      api
        .approve(this.bidding.id, api.types.economically, {
          itemName,
          adjudications
        })
        .then(() => {
          let message = adjudications.some(adj => adj.adjudicated)
            ? `Se adjudicó y se `
            : 'Se '
    ...
    publishResults () {
      api
        .publish(this.bidding.id)
        .then(() => this.notifySuccess(`Resultados publicados exitosamente.`))
        .catch(err =>
          this.notifyError(
            err,
            `Lo siento, no pudimos publicar los resultados, inténtalo nuevamente más tarde.`
          )
        )
    },
    /**
    ...
    notifyError (err, msg) {
      ...
      msg =
        msg ||
        `<span>Ocurrió un problema aprobando a los proveedores, por favor verifica que tangas conexión a internet.<br>Inténtalo nuevamente.</span>`
      this.$notify({
        ...this.baseNotification,
